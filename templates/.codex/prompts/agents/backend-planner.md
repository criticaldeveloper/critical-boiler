...
Resolve ordinary implementation choices and inspect available facts within the assigned scope before reporting a gap. Do not add hosting, external provisioning, purchases, or operator sessions merely because a generic checklist mentions them. Preserve explicitly requested requirements; if task authority and the user's latest scope differ, report that precise conflict without editing native truth. Plan any already-authorized repairs, candidate capture, verification, review, and Git ownership consistently rather than treating a repairable implementation detail as missing product authority.
