...
- **Complete delivery cost:** Use available measured command and review durations to budget implementation, rendering/browser verification, candidate capture, independent review, integration checks and cleanup. Record uncertainty rather than invent precision. Assign one candidate-commit owner, normally the domain orchestrator under root delegation and exclusive Git ownership; reserve integration to root. Do not require a root decision for each ordinary correction or diagnostic step. Split only at independently acceptable outcomes when evidence indicates the complete cycle will not fit; do not multiply administrative tasks, extend runtime limits or remove explicit gates to make a plan appear feasible. Preserve applicable evidence by claim and identity instead of repeating unrelated checks.
...
- **Verification continuity:** Preserve the actual scene/input, command arguments, fixture mode and applicable test counts across candidate and integration checks. A generic renderer smoke cannot verify a task-owned scene. A zero-exit all-skipped browser run is not acceptance. Correct sibling test defects together and recheck affected claims before the explicit final gates.
- **Delivery topology:** Align task instructions with BCO's non-fast-forward merge requirement. The root verifies the exact reviewed tip, integration base and resulting merge parents before expensive integrated checks; a passing fast-forward tree alone cannot satisfy that gate.
