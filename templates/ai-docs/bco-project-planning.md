...
BCO remains authoritative for native tasks, relationships, versions, and validation. The skill may inspect the repository and prepare a structured draft. It may preview, validate, or apply that draft only through project-planning capabilities actually exposed by BCO and only within the operator's authorization.

If no project-planning mutation capability is available, return the validated draft for operator entry in BCO. Do not connect to BCO persistence, use workflow-scoped task tools to manufacture a backlog, or claim that a draft was applied. After manual entry, reread BCO truth and validate the native graph before enabling Experimental Brain.

## Planning Rules

- Express every dependency as a native BCO relationship using stable task keys. A sentence such as “depends on Task 2” is context, not an edge.
...
Reject cycles, missing keys, self-dependencies, ambiguous prose-only dependencies, broad multi-outcome tasks, verification without observable commands or evidence, and tasks requiring unavailable capabilities without an explicit enabling dependency.
...
2. every prerequisite is a persisted native BCO dependency;
...
8. the operator confirms the plan or applied changes.
