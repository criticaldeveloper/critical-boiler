...
Failed verification or rejected review repeats scoped repair, fresh exact-tree verification, and independent review while the current workflow's existing time and specialist budgets remain. If a relevant budget is exhausted, the task remains `in-progress` with exact evidence for governor recovery. Workflow agents do not classify or clear `authority-conflict`; that state is reserved for operator/governor authority.

When BCO assigns `BCO_TEST_SERVER_PORT`, project test and dev-server commands use that exact port. For pnpm scripts that launch Vite, pass `--host` and `--port` directly after the script name—for example, `pnpm dev --host 127.0.0.1 --port $env:BCO_TEST_SERVER_PORT`—without an extra standalone `--`. The server owner records its root PID, verifies the exact listener, and stops its owned process tree before completion; agents never terminate pre-existing or unowned processes.
...
Version 2.6.9 makes candidate capture explicit: root may assign it to the domain orchestrator under exclusive Git ownership, while integration remains root-owned and existing task restrictions remain binding. Recovery continues through useful diagnostic milestones within actual budgets, and passing probes retain their limits. Successful completion requests await the governor without duplicate evidence or verification. Planning traces consumer commands through real dispatchers, scenarios and build outputs, assigning missing runner extensions before dependent acceptance. The BCO wire/role contracts remain compatible with 2.6.7. Align unfinished imported tasks separately when their prose reserves every candidate commit to root; managed sync does not amend native task authority.
