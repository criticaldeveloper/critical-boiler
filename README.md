...
Version 2.6.9 makes candidate capture explicit: root may assign it to the domain orchestrator under exclusive Git ownership, while integration remains root-owned and existing task restrictions remain binding. Recovery continues through useful diagnostic milestones within actual budgets, and passing probes retain their limits. Successful completion requests await the governor without duplicate evidence or verification. The BCO wire/role contracts remain compatible with 2.6.7. Align unfinished imported tasks separately when their prose reserves every candidate commit to root; managed sync does not amend native task authority.
